...
class RawTransactionsTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        # Now it's OK to request genesis txs!
        # assert_raises_rpc_error(-5, "The genesis block coinbase is not considered an ordinary transaction", self.nodes[0].getrawtransaction, block['merkleroot'])
        for tx in block['tx']:
            assert (self.nodes[0].getrawtransaction(tx) != "")
        ...
        assert_raises_rpc_error(-8, "Invalid parameter, duplicated pair 'token@address': %s" % address, self.nodes[0].createrawtransaction, [], multidict([(address, 1), (address, 1)]))
        assert_raises_rpc_error(-8, "Invalid parameter, duplicated pair 'token@address': %s" % address, self.nodes[0].createrawtransaction, [], [{address: 1}, {address: 1}])
