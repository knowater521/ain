                           ...
                           std::list<COutputEntry>& listSent, CAmount& nFee, const isminefilter& filter) const
{
    ...
    // Compute fee:
    CAmount nDebit = GetDebit(filter); /// @todo tokens: only for id == 0???
...
CAmount CWalletTx::GetCachableAmount(AmountType type, const isminefilter& filter, bool recalculate) const
{
    ...
    if (recalculate || !amount.m_cached[filter]) {
        amount.Set(filter, type == DEBIT ? pwallet->GetDebit(*tx, filter) : pwallet->GetCredit(*tx, filter)); /// @todo tokens: tiken id?
    }
...
bool CWallet::CreateMintTokenTransaction(interfaces::Chain::Lock& locked_chain, CMutableTransaction &origTx, CTransactionRef& tx, CAmount& nFeeRet,
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    CAmount nValue = 0;
    ReserveDestination reservedest(this);
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            if (!boost::get<CNoDestination>(&coin_control.destChange)) {
                ...
                OutputType change_type = m_default_address_type; //TransactionChangeType(/*coin_control.m_change_type ? *coin_control.m_change_type :*/ m_default_change_type, vecSend);
                if (change_type != OutputType::LEGACY) {
                    // if any destination is P2WPKH or P2WSH, use P2WPKH for the change output.
                    for (const auto& recipient : origTx.vout) {
                        // Check if any destination contains a witness program:
                        int witnessversion = 0;
                        std::vector<unsigned char> witnessprogram;
                        if (recipient.scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
                            change_type = OutputType::BECH32;
                            break;
                        }
                    }
                }
            ...
            while (true)
            {
                ...
                // vouts to the payees
                coin_selection_params.tx_noinputs_size = 11; // Static vsize overhead + outputs vsize. 4 nVersion, 4 nLocktime, 1 input count, 1 output count, 1 witness overhead (dummy, flag, stack size)
                for (const auto& txout : origTx.vout)
                {
                    // Include the fee cost for outputs. Note this is only used for BnB right now
                    coin_selection_params.tx_noinputs_size += ::GetSerializeSize(txout, PROTOCOL_VERSION); // with tokens!
                    txNew.vout.push_back(txout);
                }
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    // (because of reduced tx size) and so we should add a
                    // change output. Only try this once.
                    if (nChangePosInOut == -1 && pick_new_inputs) {
                        unsigned int tx_size_with_change = nBytes + coin_selection_params.change_output_size + 2; // Add 2 as a buffer in case increasing # of outputs changes compact size
                        CAmount fee_needed_with_change = GetMinimumFee(*this, tx_size_with_change, coin_control, nullptr);
                        CAmount minimum_value_for_change = GetDustThreshold(change_prototype_txout, txNew.nVersion, discard_rate);
                        if (nFeeRet >= fee_needed_with_change + minimum_value_for_change) {
    ...
    // Before we return success, we assume any change key will be used to prevent accidental re-use.
    reservedest.KeepDestination();
