...
const int DEFI_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString DEFI_IPC_PREFIX("defi:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/defi-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/defi-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/defi-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the defi: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(DEFI_IPC_PREFIX, Qt::CaseInsensitive)) // defi: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseDefiURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(DEFI_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(DEFI_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click defi: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start defi: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling defi: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("defi://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'defi://' is not a valid URI. Use 'defi:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(DEFI_IPC_PREFIX, Qt::CaseInsensitive)) // defi: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseDefiURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Defi address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in defi: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom defi addresses are not supported
        ...
        // Defi amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, 1, optionsModel->node().getDustRelayFee())) {  /// @todo tokens: subst "1" with correct txout version when implemented
