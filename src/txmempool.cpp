    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (!CheckFinalTx(tx, flags) || !CheckSequenceLocks(*this, tx, flags, &lp, validLP)) {
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (txin.prevout.n == 1 && (!penhancedview->CanSpend(txin.prevout.hash, nMemPoolHeight) || IsMempooledMnCreate(*this, txin.prevout.hash)))
                {
                    txToRemove.insert(it);
                    break;
                }
...
static void CheckInputsAndUpdateCoins(const CTransaction& tx, CCoinsViewCache& mempoolDuplicate, const CEnhancedCSView * mnview, const int64_t spendheight)
{
...
void CTxMemPool::xcheck(const CCoinsViewCache *pcoins, const CEnhancedCSView * mnview) const
{
    LOCK(cs);
